...
def get_model_and_buffer(args, device, sample_q):
    ...
    if args.load_path is None:
        ...
        past_buffer = init_random(args, args.buffer_size)
    else:
    ...
    f = f.to(device)
    return f, replay_buffer, past_buffer
...
def get_data(args):
    ...
    if args.num_splits > 1:
        ...
        dload_train_splits = []
        dload_train_labeled_splits = []

        labels = list(range(args.n_classes))
        num_concurrent_labels = args.n_classes // args.num_splits

        for i in range(0, args.n_classes, num_concurrent_labels):
            concurrent_labels = labels[i:i+num_concurrent_labels]
            labels_in_splits.append(concurrent_labels)

            filter_mask = np.isin(train_labels, concurrent_labels)

            split_inds = np.copy(train_inds[filter_mask])

            dset_train_x = DataSubset(dataset_fn(True, transform_train), inds=split_inds)
            dset_train_labeled_x = DataSubset(dataset_fn(True, transform_train), inds=split_inds)
            dload_train_x = DataLoader(dset_train_x, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
            dload_train_labeled_x = DataLoader(dset_train_labeled_x, batch_size=args.batch_size, shuffle=True, num_workers=4, drop_last=True)
            dload_train_labeled_x = cycle(dload_train_labeled_x)

            dload_train_splits.append(dload_train_x)
            dload_train_labeled_splits.append(dload_train_labeled_x)
    else:
        dload_train_splits = [dload_train]
        dload_train_labeled_splits = [dload_train_labeled]
    ...
    return dload_train, dload_train_labeled, dload_valid,dload_test, dload_train_splits, dload_train_labeled_splits, labels_in_splits
...
def main(args):
    ...
    # datasets
    dload_train, dload_train_labeled, dload_valid, dload_test, dload_train_splits, dload_train_labeled_splits, labels_in_splits = get_data(args)

    device = t.device('cuda' if t.cuda.is_available() else 'cpu')

    sample_q = get_sample_q(args, device)
    f, replay_buffer, past_buffer = get_model_and_buffer(args, device, sample_q)
        ...
        for i, (x_p_d, _) in tqdm(enumerate(dload_train)):
            ...
            if args.p_y_given_x_weight > 0:  # maximize log p(y | x)
                ...
                if global_iter % args.print_every == 0:
                    ...
                    neptune.send_metric('loss_p_y_given_x', x=global_iter, y=l_p_y_given_x.item())
            ...
            if args.p_x_y_weight > 0:  # maximize log p(x, y)
                ...
                if global_iter % args.print_every == 0:
                    ...
                    neptune.send_metric('f(x_p_d)', x=global_iter, y=fp)
                    neptune.send_metric('f(x_q)', x=global_iter, y=fq)
                    neptune.send_metric('l_p_x_y', x=global_iter, y=-(fp-fq))
            ...
            if args.p_y_given_x_past_weight > 0:
                ...
                if global_iter % args.print_every == 0:
                    ...
                    print('P(y|x_past) {}:{:>d} loss={:>14.9f}, acc={:>14.9f}'.format(epoch,
                                                                                 ...
                                                                                 acc_past.item()))
                    neptune.send_metric('acc_past', x=global_iter, y=acc_past.item())
            ...
            if global_iter % 100 == 0:
                if args.plot_uncond:
                    ...
                    filename_uncond = '{}/x_q_{}_{:>06d}_{:>06d}.png'.format(args.save_dir, epoch, i, global_iter)
                ...
                if args.plot_cond:  # generate class-conditional samples
                    ...
                    neptune.send_image('samples_cond', x=global_iter, y=filename_cond)
        ...
        if epoch % args.eval_every == 0 and (args.p_y_given_x_weight > 0 or args.p_x_y_weight > 0):
            ...
            with t.no_grad():
                ...
                if correct > best_valid_acc:
                    ...
                    neptune.send_metric('best_valid', x=global_iter, y=best_valid_acc)
                # test set
...
if __name__ == "__main__":
    ...
    parser.add_argument("--p_y_given_x_past_weight", type=float, default=0.)
    ...
    if use_neptune:
        ...
        exp = neptune.create_experiment(params=vars(args), name="exp")
